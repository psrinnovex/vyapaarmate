...
type CustomerGoogleMarker = {
  addListener: (eventName: string, handler: () => void) => GoogleMapsListener;
  getPosition: () => GoogleLatLng | undefined;
  setMap: (map: CustomerGoogleMap | null) => void;
  setPosition: (position: GoogleLatLngLiteral | null) => void;
  setAnimation?: (animation: unknown) => void;
};

type CustomerGoogleCircle = {
  setMap: (map: CustomerGoogleMap | null) => void;
};

type GoogleMapsExtras = {
  Animation?: {
    DROP?: unknown;
  };
  Circle?: new (options: Record<string, unknown>) => CustomerGoogleCircle;
};
type GoogleMapsNamespace = NonNullable<typeof window.google>["maps"] & GoogleMapsExtras;
...
type CustomerLocationMapPickerProps = {
  value: MapCoordinates | null;
  onChange: (coordinates: MapCoordinates | null) => void;
...
export function CustomerLocationMapPicker({
  ...
  const mapInstanceRef = useRef<CustomerGoogleMap | null>(null);
  const markerRef = useRef<CustomerGoogleMarker | null>(null);
  const radiusRef = useRef<CustomerGoogleCircle | null>(null);
  ...
  const animateMarker = useCallback(() => {
    ...
    const mapsExtras = window.google?.maps as GoogleMapsNamespace | undefined;
    if (!marker?.setAnimation || !mapsExtras?.Animation?.DROP) return;
    marker.setAnimation(mapsExtras.Animation.DROP);
  }, []);
  ...
  const setSelectedLocation = useCallback(
    (coordinates: MapCoordinates | null) => {
      ...
      if (coordinates) {
        focusMapOnLocation(coordinates);
        animateMarker();
      }
    },
    [animateMarker, focusMapOnLocation, onChange]
  );
      ...
      .then(() => {
        ...
        const businessCenter =
          ...
          Number.isFinite(businessLongitude)
            ...
            : null;
        const startingLocation = initialValueRef.current ?? businessCenter ?? DEFAULT_MAP_CENTER;
        const map = new window.google.maps.Map(mapElement, {
          center: toGoogleCoordinates(startingLocation),
          zoom: initialValueRef.current || businessCenter ? SELECTED_LOCATION_ZOOM : DEFAULT_MAP_ZOOM,
          clickableIcons: false,
          fullscreenControl: false,
          mapTypeControl: false,
          streetViewControl: false
        }) as CustomerGoogleMap;
        const mapsExtras = window.google.maps as GoogleMapsNamespace;
        const marker = new window.google.maps.Marker({
          map,
          draggable: true,
          title: "Service location",
          animation: mapsExtras.Animation?.DROP
        }) as CustomerGoogleMarker;

        if (initialValueRef.current) {
          marker.setPosition(toGoogleCoordinates(initialValueRef.current));
        }

        if (businessCenter && serviceRadiusKm > 0 && mapsExtras.Circle) {
          radiusRef.current = new mapsExtras.Circle({
            map,
            center: toGoogleCoordinates(businessCenter),
        ...
        map.addListener("click", (event?: GoogleMapClickEvent) => {
          ...
          setSelectedLocation(fromGoogleLatLng(event.latLng));
        });
        ...
        marker.addListener("dragend", () => {
          ...
          if (!position) return;
          setSelectedLocation(fromGoogleLatLng(position));
        });
  ...
  }, [apiKey, businessLatitude, businessLongitude, serviceRadiusKm, setSelectedLocation]);

  useEffect(() => {
    const marker = markerRef.current;
    if (!marker) return;
    marker.setPosition(value ? toGoogleCoordinates(value) : null);
  }, [value]);

  const searchLocations = useCallback(async () => {
    const query = searchQuery.trim() || suggestedQuery.trim();
    if (!query) {
      setSearchResults([]);
  ...
  return (
    <div className={cn("grid gap-3", className)}>
      ...
      {searchResults.length > 0 && (
        <div className="grid max-h-52 gap-1 overflow-y-auto rounded-lg border border-line bg-white p-2 shadow-sm">
      ...
      <div
        role="application"
        aria-label="Google service location map"
        className="relative h-72 overflow-hidden rounded-lg border border-line bg-slate-100 shadow-inner"
      >
        ...
        {value && (
          <div className="pointer-events-none absolute left-3 top-3 z-20 rounded-full border border-emerald/20 bg-white/95 px-3 py-1 text-xs font-bold text-emerald shadow-sm">
            {dragging ? "Moving pin" : "Location pinned"}
          </div>
        )}
        <div className="absolute right-3 top-3 z-20 grid gap-2">
          ...
          <button
            ...
            title="Center pin"
            disabled={!value || mapStatus !== "ready"}
            className="grid size-9 place-items-center rounded-lg border border-line bg-white text-ink shadow-sm transition hover:border-ocean/30 disabled:cursor-not-allowed disabled:opacity-50"
            onClick={() => {
              if (value) focusMapOnLocation(value);
            }}
          >
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto] sm:items-stretch">
        <div className="rounded-lg border border-line bg-mist p-3">
          ...
          <p className="mt-1 font-mono text-sm font-semibold text-ink">{value ? formatMapCoordinate(value.latitude) : "Not set"}</p>
        </div>
        <div className="rounded-lg border border-line bg-mist p-3">
          <p className="text-xs font-semibold uppercase text-slate-500">Longitude</p>
          <p className="mt-1 font-mono text-sm font-semibold text-ink">{value ? formatMapCoordinate(value.longitude) : "Not set"}</p>
        </div>
        <Button
          type="button"
          variant="secondary"
